...
const StudentCoordinatorsList = () => {
  ...
  const fetchCoordinators = async () => {
    try {
      setLoading(true);
      const res = await api.get('/coordinators/student');
  ...
  const fetchRequests = async () => {
    try {
      ...
      setRequests(res.data.filter(r => r.role === 'student_coordinator' && r.status === 'Pending'));
    } catch (error) {
      console.error('Error fetching student coordinator requests:', error);
    } finally {
  ...
  const handleApprove = async (id) => {
    if (window.confirm('Are you sure you want to approve this request? This will activate their Student Coordinator profile.')) {
      try {
      ...
      <div style={{ flexGrow: 1, maxWidth: '1000px', margin: '0 auto', width: '100%', padding: '120px 24px 60px 24px' }}>
        <div style={{ marginBottom: '35px' }}>
          ...
          <p style={{ margin: '0', color: 'var(--text-muted)', fontSize: '15px' }}>
            View and assign student leads for different campus events
          </p>
        </div>

        {/* Student Coordinator Requests Section */}
        {requests.length > 0 && (
          <div style={{ background: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--light-border)', boxShadow: 'var(--shadow-md)', padding: '30px', marginBottom: '30px' }}>
            <h2 style={{ fontSize: '20px', fontFamily: 'Outfit, sans-serif', fontWeight: 700, color: 'var(--text-dark)', margin: '0 0 20px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              Pending Student Coordinator Requests
            </h2>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--light-border)', boxShadow: 'var(--shadow-md)', padding: '30px' }}>
          ...
          {loading ? (
            <h3 style={{ color: 'var(--text-muted)', textAlign: 'center', padding: '40px 0' }}>Loading student leads...</h3>
              ...
              <table className="table table-hover" style={{ margin: '0', border: 'none', boxShadow: 'none', width: '100%' }}>
                ...
                <tbody>
                  {coordinators.map((row, idx) => (
                    <tr key={idx}>
                      <td style={{ fontWeight: '600', color: '#4f46e5' }}>{row.st_name}</td>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <Link to={`/admin/coordinators/student/update?id=${row.event_id}`} style={{
                          display: 'inline-flex',
                          alignItems: 'center',
                          gap: '6px',
                          textDecoration: 'none',
            ...
            <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>
              <h3>No student coordinator records found</h3>
            </div>
          )}
        </div>
      </div>
...
export default StudentCoordinatorsList;
