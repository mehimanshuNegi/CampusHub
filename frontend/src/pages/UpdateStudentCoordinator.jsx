...
const UpdateStudentCoordinator = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put(`/api/coordinators/student/${id}`, {
        st_name,
        phone
      });
      alert(res.data.message || 'Updated Successfully');
      navigate('/admin/coordinators/student');
    } catch (error) {
        ...
        <div style={{ width: '100%', maxWidth: '460px' }}>
          ...
          <form onSubmit={handleSubmit} style={{ margin: '0', border: '1px solid var(--light-border)', boxShadow: 'var(--shadow-md)', borderRadius: '12px', background: 'var(--bg-card)' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div>
                <label style={{ fontWeight: '600', color: 'var(--text-dark)' }}>Student Coordinator Phone</label>
                <input
                  type="text"
                  className="form-control"
...
export default UpdateStudentCoordinator;
